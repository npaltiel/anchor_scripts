...
def create_table_with_primary_key(table_name, df, conn):
    # Dynamically generate the CREATE TABLE SQL statement
    columns = ", ".join([f"{col} TEXT" if col != "VisitID" else f"{col} INTEGER PRIMARY KEY" for col in df.columns])
    create_table_sql = f"""
...
#     "C:\\Users\\nochum.paltiel\\OneDrive - Anchor Home Health care\\Documents\\General Information\\Visits\\Visit_Report_Lehigh.csv",
...
visits_df = pd.read_csv(
    "C:\\Users\\nochum.paltiel\\OneDrive - Anchor Home Health care\\Documents\\General Information\\Visits\\Visit_Report_Aug_Nov.csv")
...
# create_table_with_primary_key('visits', visits_df, conn)
