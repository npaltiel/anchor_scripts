...
aggregation_functions = {col: 'first' for col in df_patients.columns if col != concat_column}
aggregation_functions[concat_column] = lambda x: ', '.join(map(str, filter(pd.notna, x)))
...
cur_df['Address'] = cur_df['Address Line 1'].fillna('').str.title() + ' ' + cur_df['Address Line 2'].fillna('').str.title()
cur_df['City, State'] = cur_df['City'].fillna('').str.title() + ', ' + cur_df['State'].fillna('').str.title()
nhtd_df = cur_df[['Last Name', 'First Name', 'Address', 'City, State', 'Zip', 'DOB', 'Diagnosis Code -Clinical', 'Gender', 'AdmissionID', 'MedicaidNo']].copy()

nhtd_df['Visit Date'] = [f'{current.month}/1/{current.year}' if nhtd_df['MedicaidNo'][i] in prev_ids else "" for i in range(len(nhtd_df))]
