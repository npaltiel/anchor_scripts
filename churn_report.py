...
df_4 = pd.read_csv(
    "C:\\Users\\nochum.paltiel\\OneDrive - Anchor Home Health care\\Documents\\Churn Report\\Visit_Report_Aug_Nov.csv")
...
visits_df['Branch_Updated'] = [
    'Baby' if pd.notna(visits_df['Date of Birth'][i]) and
              datetime.strptime(visits_df['Date of Birth'][i].strip(), "%m/%d/%Y").date() >= pd.Timestamp(
