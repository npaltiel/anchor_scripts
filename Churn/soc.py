...
# --- Previous period: keep the most recent visit per UniqueID ---
...
# --- Include MLTC -> NHTD transfers even if they appeared before ---

# Map each UniqueID's previous ContractType (from prev_df) onto the current period
prev_contract_by_id = (
    prev_df
    .set_index('UniqueID')['ContractType']
)

# Work on a copy to avoid chained assignment pitfalls
cur_df = cur_df.copy()

# Add previous contract type for comparison
cur_df['PrevContractType'] = cur_df['UniqueID'].map(prev_contract_by_id)

# Identify transfers: previously MLTC, now NHTD
transfer_mask = (cur_df['PrevContractType'] == 'MLTC') & (cur_df['ContractType'] == 'NHTD')

# Base mask = new SOCs: UniqueID not seen before
new_mask = ~cur_df['UniqueID'].isin(prev_ids)

# Final inclusion mask: either brand-new SOCs OR MLTC→NHTD transfers
final_mask = new_mask | transfer_mask

# For transfers, override Branch_Updated as requested
cur_df.loc[transfer_mask, 'Branch_Updated'] = 'Transferred to NHTD'
